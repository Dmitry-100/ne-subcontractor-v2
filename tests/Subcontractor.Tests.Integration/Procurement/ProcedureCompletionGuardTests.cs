...
public sealed class ProcedureCompletionGuardTests
{
    [Fact]
    ...
    [Fact]
    public async Task TransitionToCompleted_WithMultipleContracts_ShouldThrow()
    {
        await using var db = TestDbContextFactory.Create("proc-user");
        var (lotId, procedureId) = await SeedDecisionMadeProcedureAsync(db);

        await db.Set<Contract>().AddRangeAsync(
            CreateContract(lotId, procedureId, "CTR-001"),
            CreateContract(lotId, procedureId, "CTR-002"));
        await db.SaveChangesAsync();
        ...
        var error = await Assert.ThrowsAsync<InvalidOperationException>(() => service.TransitionAsync(procedureId, request));
        Assert.Equal("Procedure has multiple contracts. Keep only one contract before completion.", error.Message);
    }
    ...
    private static async Task<(Guid LotId, Guid ProcedureId)> SeedDecisionMadeProcedureAsync(IApplicationDbContext db)
    {
        var lot = new Lot
        {
            Code = "LOT-001",
