...
const controllersModule = require(path.resolve(
    __dirname,
...
test("contracts runtime controllers: validates required dependencies", () => {
    assert.throws(function () {
        controllersModule.createControllersComposition({
            foundation: {},
            elements: {},
            modules: {}
        });
...
test("contracts runtime controllers: composes controllers and wires orchestration callbacks", async () => {
    const events = {
    ...
    const foundation = {
        ...
        apiClient: { id: "contracts-api-client" },
        ...
        appendFilterHint: function (message) { return `${message}#hint`; },
        clearUrlFilters: function () {
            const currentUrl = foundation.hostWindow.location.href;
            events.clearUrlFiltersCalls.push(currentUrl);
            foundation.hostWindow.location.assign(`${currentUrl}#cleared`);
        },
        setStatus: function () {},
        ...
        getStatusRank: function (status) {
            return ["Draft", "OnApproval", "Signed", "Active", "Closed"].indexOf(String(status || ""));
        },
